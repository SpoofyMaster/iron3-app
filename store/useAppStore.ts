...
import { IronmanEvent } from "@/lib/ironmanEvents";
import {
  mockUser,
  mockActivities,
  mockPersonalBests,
...
interface AppState {
  ...
  // Race goal & PREP plan
  selectRaceEvent: (event: IronmanEvent, autoGeneratePlan: boolean) => void;
  clearRaceEvent: () => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  // ── Race goal & PREP plan ────────────────────────────────────────────────
  selectRaceEvent: (event: IronmanEvent, autoGeneratePlan: boolean) => {
    const state = get();
    ...
    set({ selectedRaceEvent: event, prepPlan: plan });
  },

  clearRaceEvent: () => set({ selectedRaceEvent: null, prepPlan: null }),
  ...
  fetchActivities: async (userId: string) => {
    try {
      ...
      // New user — no activities yet → start at Iron, 0 XP
      ...
      if (data && data.length > 0) {
        ...
        // Per-discipline last activity dates (for XP decay)
        const lastSwimDate = swimActs.length > 0
          ? swimActs.reduce((a, b) => a.date > b.date ? a : b).date
          : null;
        const lastBikeDate = bikeActs.length > 0
          ? bikeActs.reduce((a, b) => a.date > b.date ? a : b).date
          : null;
        const lastRunDate = runActs.length > 0
          ? runActs.reduce((a, b) => a.date > b.date ? a : b).date
          : null;
